...
| **Live demo** | A human can watch the whole stack work: DHT forms, a viewer discovers a publisher by looking the feed key up as a *topic* (the key is not the publisher's node id), punches a connection, streams a signed feed, verifies every frame | `transfer` |
...
- `cargo run -p transfer --example stream` — the whole stack: a viewer looks the feed key up as a *topic* to discover the (random-id) publisher — reaching the key *as a node* finds no one — then punches a connection and streams a signed feed back over real UDP, verifying every frame — no server in the path.

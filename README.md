...
| **Live demo** | A human can watch the whole stack work: DHT forms, a viewer discovers a publisher by looking up a *blinded, rotating topic* `H(feed key ‖ epoch)` (the key is neither the publisher's node id nor the cleartext topic), punches a connection, streams a signed feed, verifies every frame | `transfer` |
